...
function onScrollSliderParallax() {
	if (!requesting) {
		...
		requestAnimationFrame(function(){
			KINARA.slider.sliderParallax();
			KINARA.slider.sliderElementsFade();
		});
	}
	killRequesting();
}
...
(function($){
	...
	KINARA.initialize = {
		...
		init: function(){
			...
			KINARA.initialize.responsiveClasses();
			KINARA.initialize.stickyElements();
			KINARA.initialize.goToTop();
			KINARA.initialize.lazyLoad();
			KINARA.initialize.fullScreen();
			KINARA.initialize.verticalMiddle();
			KINARA.initialize.lightbox();
			KINARA.initialize.resizeVideos();
			KINARA.initialize.imageFade();
			KINARA.initialize.pageTransition();
			KINARA.initialize.dataResponsiveClasses();
			KINARA.initialize.dataResponsiveHeights();
			KINARA.initialize.stickFooterOnSmall();
			KINARA.initialize.stickyFooter();
				...
				$lightboxAjaxEl.magnificPopup({
					...
					callbacks: {
						ajaxContentAdded: function(mfpResponse) {
							KINARA.widget.loadFlexSlider();
							KINARA.initialize.resizeVideos();
							KINARA.widget.masonryThumbs();
						},
						open: function() {
		...
		topScrollOffset: function() {
			...
			if( ( $body.hasClass('device-xl') || $body.hasClass('device-lg') ) && !KINARA.isMobile.any() ) {
		...
		setFullColumnWidth: function( element ){
			...
			if( element.hasClass('portfolio-full') ) {
				var columns = KINARA.initialize.defineColumns( element ),
	...
	KINARA.header = {
		...
		init: function(){
			...
			KINARA.header.superfish();
			KINARA.header.menufunctions();
			KINARA.header.fullWidthMenu();
			KINARA.header.overlayMenu();
			KINARA.header.stickyMenu();
			KINARA.header.stickyPageMenu();
			KINARA.header.sideHeader();
			KINARA.header.sidePanel();
			KINARA.header.onePageScroll();
			KINARA.header.onepageScroller();
			KINARA.header.logo();
			KINARA.header.topsearch();
			KINARA.header.topcart();
		...
		stickyMenu: function( headerOffset ){
			if ($window.scrollTop() > headerOffset) {
				...
				KINARA.header.removeStickyness();
			}
		},
		...
		removeStickyness: function(){
			if( $header.hasClass('sticky-header') ){
				...
				if( !$headerWrap.hasClass('force-not-dark') ) { $headerWrap.removeClass('not-dark'); }
				KINARA.slider.swiperSliderMenu();
				KINARA.slider.revolutionSliderMenu();
			}
			...
			if( $onePageMenuEl.length > 0 ){
				...
				$onePageMenuEl.find('a[data-href]').off( 'click' ).on( 'click', function(){
					...
					if( $( divScrollToAnchor ).length > 0 ) {
						...
						if( !onePageOffset ) {
							var onePageOffsetG = KINARA.initialize.topScrollOffset();
						} else {
		...
		onepageScroller: function(){
			$onePageMenuEl.find('li').removeClass('current');
			$onePageMenuEl.find('a[data-href="#' + KINARA.header.onePageCurrentSection() + '"]').parent('li').addClass('current');
		},
	...
	KINARA.slider = {
		...
		init: function() {
			...
			KINARA.slider.sliderParallaxDimensions();
			KINARA.slider.sliderRun();
			KINARA.slider.sliderParallax();
			KINARA.slider.sliderElementsFade();
			KINARA.slider.captionPosition();
		...
		sliderParallax: function(){
			...
			var parallaxOffsetTop = KINARA.slider.sliderParallaxOffset(),
		...
		swiperSliderMenu: function( onWinLoad ){
			...
			if( $body.hasClass('device-xl') || $body.hasClass('device-lg') || ( $header.hasClass('transparent-header-responsive') && !$body.hasClass('primary-menu-open') ) ) {
				...
				KINARA.slider.headerSchemeChanger(activeSlide, onWinLoad);
			}
		},
	...
	KINARA.portfolio = {
		...
		arrange: function(){
			if( $portfolio.length > 0 ) {
				$portfolio.each( function(){
					var element = $(this);
					KINARA.initialize.setFullColumnWidth( element );
				});
			}
		},
		...
		ajaxload: function(){
			$('.portfolio-ajax .portfolio-item a.center-icon').off( 'click' ).on( 'click', function(e) {
				...
				if( !$(this).parents('.portfolio-item').hasClass('portfolio-active') ) {
					KINARA.portfolio.loadItem(portPostId, prevPostPortId);
				}
				e.preventDefault();
			});
		},
		...
		newNextPrev: function( portPostId ){
			var portNext = KINARA.portfolio.getNextItem(portPostId);
			var portPrev = KINARA.portfolio.getPrevItem(portPostId);
		...
		loadItem: function( portPostId, prevPostPortId, getIt ){
			...
			var portPrev = KINARA.portfolio.getPrevItem(portPostId);
			if(getIt == false) {
				KINARA.portfolio.closeItem();
				$portfolioAjaxLoader.fadeIn();
				var portfolioDataLoader = $('#' + portPostId).attr('data-loader');
				$portfolioDetailsContainer.load(portfolioDataLoader, { portid: portPostId, portnext: portNext, portprev: portPrev },
				function(){
					KINARA.portfolio.initializeAjax(portPostId);
					KINARA.portfolio.openItem();
		...
		openItem: function(){
			...
			if( noOfImages > 0 ) {
				$portfolioDetails.find('img').on('load', function(){
					noLoaded++;
					var topOffsetScroll = KINARA.initialize.topScrollOffset();
	...
	KINARA.widget = {
		...
		init: function(){
			...
			KINARA.widget.animations();
			KINARA.widget.youtubeBgVideo();
			KINARA.widget.tabs();
			KINARA.widget.tabsJustify();
			KINARA.widget.tabsResponsive();
			KINARA.widget.tabsResponsiveResize();
			KINARA.widget.toggles();
			KINARA.widget.accordions();
			KINARA.widget.counter();
			KINARA.widget.roundedSkill();
			KINARA.widget.progress();
			KINARA.widget.twitterFeed();
			KINARA.widget.flickrFeed();
			KINARA.widget.instagramPhotos( '5834720953.1677ed0.a0a26ba4c90845f9a844d64c316bf77a', '8e000fefe3024b2ead6a50ff005bf036' );
			KINARA.widget.dribbbleShots( '012d3d72d12f93e1d41a19195d7da2fc87e6b5afa48a184256e398eb793cfe56' );
			KINARA.widget.navTree();
			KINARA.widget.textRotater();
			KINARA.widget.carousel();
			KINARA.widget.linkScroll();
			KINARA.widget.ajaxForm();
			KINARA.widget.subscription();
			KINARA.widget.stickySidebar();
			KINARA.widget.cookieNotify();
			KINARA.widget.cartQuantity();
			KINARA.widget.extras();
				...
				$flexSliderEl.each(function() {
					...
					$flexsSlider.flexslider({
						...
						start: function(slider){
							...
							KINARA.portfolio.portfolioDescMargin();
						},
						after: function(){
		...
		html5Video: function(){
			...
			if( videoEl.length > 0 ) {
				videoEl.each(function(){
					...
					if( KINARA.isMobile.any() && !element.hasClass('no-placeholder') ) {
						...
						if( $(this).next().is(':hidden') ) {
							...
							$(this).toggleClass('acctitlec').next().stop().slideDown("normal", function(){
								if( ( $body.hasClass('device-sm') || $body.hasClass('device-xs') ) && element.hasClass('scroll-on-open') ) {
									$('html,body').stop(true).animate({
										'scrollTop': clickTarget.offset().top - ( KINARA.initialize.topScrollOffset() - 40 )
									}, 800, 'easeOutQuad' );
								}
							});
						}
		...
		counter: function(){
			...
			if( $counterEl.length > 0 ){
				$counterEl.each(function(){
					...
					if( $body.hasClass('device-xl') || $body.hasClass('device-lg') ){
						element.appear( function(){
							KINARA.widget.runCounter( element, counterElementComma );
						},{accX: 0, accY: -120},'easeInCubic');
					} else {
						KINARA.widget.runCounter( element, counterElementComma );
					}
					...
					if( $body.hasClass('device-xl') || $body.hasClass('device-lg') ){
						...
						element.appear( function(){
							if (!element.hasClass('skills-animated')) {
								...
								KINARA.widget.runRoundedSkills( element, properties );
							...
							element.find(".slider-wrap").html(sm_format_twitter3(tweets)).promise().done( function(){
								var timer = setInterval(function(){
									if( element.find('.slide').length > 1 ) {
										...
										var t = setTimeout( function(){ KINARA.widget.loadFlexSlider(); }, 500);
										clearInterval(timer);
									}
								},500);
							});
		...
		masonryThumbs: function(){
			...
			if( $masonryThumbsEl.length > 0 ){
				$masonryThumbsEl.each( function(){
					...
					KINARA.widget.masonryThumbsArrange( masonryItemContainer );
				});
			}
		},
		...
		linkScroll: function(){
			$("a[data-scrollto]").off( 'click' ).on( 'click', function(){
				...
				if( !divScrollOffset ) { divScrollOffset = KINARA.initialize.topScrollOffset(); }
						...
						$(form).ajaxSubmit({
							...
							success: function( data ) {
								...
								if( elementAlert == 'inline' ) {
									...
									KINARA.widget.notifications( elementResult );
								}
	...
	KINARA.isMobile = {
		...
		any: function() {
			return (KINARA.isMobile.Android() || KINARA.isMobile.BlackBerry() || KINARA.isMobile.iOS() || KINARA.isMobile.Opera() || KINARA.isMobile.Windows());
		}
	};

	KINARA.documentOnResize = {

		init: function(){

			var t = setTimeout( function(){
				KINARA.header.topsocial();
				KINARA.header.fullWidthMenu();
				KINARA.header.overlayMenu();
				KINARA.initialize.fullScreen();
				KINARA.initialize.dataResponsiveHeights();
				KINARA.initialize.verticalMiddle();
				KINARA.initialize.testimonialsGrid();
				KINARA.initialize.stickFooterOnSmall();
				KINARA.initialize.stickyFooter();
				KINARA.slider.sliderParallaxDimensions();
				KINARA.slider.captionPosition();
				KINARA.portfolio.arrange();
				KINARA.portfolio.portfolioDescMargin();
				KINARA.widget.tabsResponsiveResize();
				KINARA.widget.tabsJustify();
				KINARA.widget.html5Video();
				KINARA.widget.masonryThumbs();
				KINARA.initialize.dataResponsiveClasses();
	...
	KINARA.documentOnReady = {

		init: function(){
			KINARA.initialize.init();
			KINARA.header.init();
			if( $slider.length > 0 || $sliderElement.length > 0 ) { KINARA.slider.init(); }
			if( $portfolio.length > 0 ) { KINARA.portfolio.init(); }
			KINARA.widget.init();
			KINARA.documentOnReady.windowscroll();
		},
		...
		windowscroll: function(){
			...
			KINARA.header.stickyMenu( headerWrapOffset );
			KINARA.header.stickyPageMenu( pageMenuOffset );

			$window.on( 'scroll', function(){

				KINARA.initialize.goToTopScroll();
	...
	KINARA.documentOnLoad = {

		init: function(){
			KINARA.slider.captionPosition();
			KINARA.slider.swiperSliderMenu(true);
			KINARA.slider.revolutionSliderMenu(true);
			KINARA.initialize.testimonialsGrid();
			KINARA.initialize.verticalMiddle();
			KINARA.initialize.stickFooterOnSmall();
			KINARA.initialize.stickyFooter();
			KINARA.portfolio.gridInit( $gridContainer );
			KINARA.portfolio.filterInit();
			KINARA.portfolio.shuffleInit();
			KINARA.portfolio.arrange();
			KINARA.portfolio.portfolioDescMargin();
			KINARA.widget.parallax();
			KINARA.widget.loadFlexSlider();
			KINARA.widget.html5Video();
			KINARA.widget.masonryThumbs();
			KINARA.header.topsocial();
			KINARA.header.responsiveMenuClass();
			KINARA.initialize.modal();
		}
	...
	$(document).ready( KINARA.documentOnReady.init );
	$window.on( 'load', KINARA.documentOnLoad.init );
	$window.on( 'resize', KINARA.documentOnResize.init );
